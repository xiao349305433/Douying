...
public class DragPhotoActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        List<String> list= new Gson().fromJson(getIntent().getStringExtra("mList"), new TypeToken<List<String>>() {}.getType());
        mList.add(list.get(index));

        mPhotoViews = new DragPhotoView[mList.size()];

        for (int i = 0; i < mPhotoViews.length; i++) {
            mPhotoViews[i] = (DragPhotoView) View.inflate(this, R.layout.item_photovp, null);
            ImgLoadUtils.loadImage(this,mList.get(i),mPhotoViews[i]);
